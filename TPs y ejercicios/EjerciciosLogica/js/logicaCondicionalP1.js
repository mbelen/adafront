...
console.log("");
console.log("Ej 1 con estructura de control (if)");
...
console.log("");
console.log("Ej 1 con una expresion");
// " ? algoEnElMedio : " operador ternario
// " ?: " operador Elvis

// BOOLEANO ? TIPOA : TIPOA
// CONVERSION IMPLICITA:
// 0 es false
// anything else es true
console.log(x % 2 == 0 ? "x es par" : "x es impar");
console.log(!(x % 2 == 0) ? "x es impar" : "x es par");
console.log(x % 2 != 0 ? "x es impar" : "x es par");
console.log(x % 2 ? "x es impar" : "x es par");


// X ? Y : Z // x entonces y, sino z
// X ?: Z === X ? X : Z // x entonces x, sino z
...
function resto2(x) {  // eliminamos parentesis por precedencia
	if (x % 2 == 0) { 
		var parImpar = "par";
	} else {
		var parImpar = "impar";
	}
	console.log("x es ", x, " y es ", parImpar); // sacamos el console log porque hace lo mismo en ambas ramas
}
function resto3(x) {
	if (x % 2 != 0) {  // invertimos la condicion e invertimos el THEN / ELSE
		var parImpar = "impar";
	} else {
		var parImpar = "par";
	}
...
function resto4(x) {
	if (x % 2) {  // tiramos el != 0 por conversion implicita
		var parImpar = "impar";
	} else {
		var parImpar = "par";
	}
...
function resto5(x) {
	var parImpar = x % 2 ? "impar" : "par";  // transformamos un if que solo setea valores a un ternario
	console.log("x es ", x, " y es ", parImpar);
}
function resto6(x) {
	console.log("x es ", x, " y es ", x % 2 ? "impar" : "par"); // reemplazamos variable por su definicion porque solo la usamos una vez
}
...
if ((y > 0) && (y < 8)) {
	switch (y) {
		case (y = 1): dia = "Lunes";  break;
		case (y = 2): dia = "Martes"; break;
		case (y = 3): dia = "Miercoles"; break;
		case (y = 4): dia = "Jueves"; break;
		case (y = 5): dia = "Viernes"; break;
		case (y = 6): dia = "Sabado"; break;
		case (y = 7): dia = "Domingo"; break;
	}
...
switch (true) {
	...
	case 3 == y: dia = "Miercoles"; break;
	case 4 == y: dia = "Jueves"; break;
	case 5 == y: dia = "Viernes"; break;
	case 6 == y: dia = "Sabado"; break;
	case 7 == y: dia = "Domingo"; break;
	default: dia = "un numero no valido"; break;
}
console.log(y + " es " + dia);
